...
public class ex09 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while(sc.hasNextLine()){
            s = sc.nextLine();//MyIO.readLine();
            if(s.length() == 3){
                if(s.charAt(0) == 'I')
                    a.inserir(s.charAt(2));
                else{
                    if(a.pesquisar(s.charAt(2)))
                        System.out.println(s.charAt(2) + " existe");
                    else
                        System.out.println(s.charAt(2) + " nao existe");
                } 
            }else{
                if(s.equals("INFIXA")){
                    a.caminharCentral();
                    System.out.println();
                }else if (s.equals("PREFIXA")){// == "PREFIXA"){
                    a.caminharPre();
                    System.out.println();
